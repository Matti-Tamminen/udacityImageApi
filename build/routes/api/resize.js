...
resize.get('/', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var root, filepath, thumbpath, _a, name, width, height, pics;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                root = path.resolve('./');
                filepath = path.join(root, 'build', 'full');
                thumbpath = path.join(root, 'build', 'thumb');
                _a = req.query, name = _a.name, width = _a.width, height = _a.height;
                pics = [];
                fs.readdir(thumbpath, function (err, files) {
                    if (err) {
                ...
                if (!!pics.includes("" + name + width + "x" + height + ".jpg")) return [3 /*break*/, 2];
                return [4 /*yield*/, sharp(path.join(filepath, name + ".jpg"))
            ...
            case 2:
                ...
                fs.readFile(path.join(thumbpath, "" + name + width + "x" + height + ".jpg"), function (err, img) {
                    if (err) {
